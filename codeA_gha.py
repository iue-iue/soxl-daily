# ============================================================
# 毎日の判定 v7.6（取得時刻 / 実行帯ガード / 当日除外 / FP行
#                + 終値の二重確認 / 境界警告 / C2併記
...
#   ★v7.6〈2026-09-20〉: ③のセル係数を 1.0 → 0.75 に（CELL3 として先頭に定数化）。`03` Q62／コードQ43 v2:
#     ③=1.0 は較正後 DD95 −10.14%（許容 −10.0% 超）・③=0.75 は −9.65%。判定ロジックは不変・変わるのは③のサイズだけ
# ============================================================
...
BASE_SIZE = 14
# ★ ③（SOXS）のセル係数。2026-09-20 まで 1.0 → 2026-09-21 以降 0.75（`03` Q62・`04` §7 v5.26）
#   運用（ここ）とサイズ導出（`05` コードQ35/Q38/Q43 v2）を同じ値にそろえるための定数。A+B/②=1.0・A単独/B単独=0.5 は従来どおり
CELL3 = 0.75
# ############################################################
...
if sig1 or lowc:
    ...
    print("シグナル: ③  SOXS 指値 %.2f（±0%%）※通常セッション限定   サイズ %d%% x %.2f x %.2f = 資金の %.2f%%" % (SS["C"].iloc[-1], BASE_SIZE, CELL3, volc, BASE_SIZE*CELL3*volc))
else:
